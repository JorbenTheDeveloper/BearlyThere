...
public class CrowSounds : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        crowSounds = GetComponent<AudioSource>();
        crowSounds.Play();
    }
    ...
    void Update()
    {
        ...
        if (distanceToPlayer <= audioRange)
        {
            ...
            crowSounds.volume = maxVolume * volume; // Set the volume of the bee sound
        }
        else
        {
            crowSounds.volume = 0f; // Player is outside the audio range, set volume to 0
        }
